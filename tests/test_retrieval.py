...
Pour chaque question de test, on connait l'article attendu. On vérifie qu'il
remonte dans le top-k du retrieval hybride. Si ce n'est pas le cas, le
problème vient du chunking, de l'embedding ou du corpus -- pas du LLM.

Ces mêmes questions sont réutilisées comme jeu d'evaluation au Jalon 4
(vérifier que la réponse finale cite bien le bon article).

Lancer avec : python -m pytest tests/test_retrieval.py -v
(nécessite que la base ait déjà ete indexee : python -m src.cli index)
"""
...
TEST_CASES = [
    ...
    ("Quelle est la durée du preavis en cas de licenciement ?", "L1234-1"),
