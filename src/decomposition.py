"""
Decomposition de requete (Jalon 6, amélioration).
...
question entière melange les signaux et peut faire remonter des chunks qui
...
independantes. Chaque sous-question est ensuite traitee séparément par le
pipeline de recherche (HyDE + hybride), et les contextes récupérés sont
fusionnes avant l'appel de génération final.

Compromis assume (a documenter dans le compte rendu) : cela ajoute un appel
LLM et potentiellement plusieurs recherches supplémentaires -> latence accrue.
...
DECOMPOSITION_SYSTEM_PROMPT = (
    ...
    "d'origine en contient plusieurs. Si elle est déjà simple, renvoie-la "
...
def looks_compound(question: str) -> bool:
    ...
    déjà simple (economie de latence/couts, cf. compromis documente)."""
...
def decompose_question(question: str) -> list:
    ...
    question est simple, ou si l'appel LLM échoue."""
