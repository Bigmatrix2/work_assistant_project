...
Design clé (a expliquer dans le README, Q2 et Q4/Q5) :

  - Le prompt système interdit d'inventer un numéro d'article et impose de
  ...
  - MAIS on ne fait pas une confiance aveugle au prompt : l'avertissement
    ...
    seulement demande au LLM. C'est la réponse a la contrainte du sujet
    ("un assistant qui l'oublie, même une fois sur dix, échoue") : un LLM
  ...
  - Si aucun chunk pertinent n'est retrouve (corpus vide sur le sujet), on
    NE FAIT MÊME PAS d'appel LLM : on renvoie un refus canonique. Cela
  ...
  - Le prompt gere explicitement les cas conditionnels (taille d'entreprise,
    convention collective) en demandant une réponse générale assortie de
    reserves, et la frontiere conseil juridique / information factuelle en
    demandant au modèle d'orienter vers un professionnel des qu'une question
...
1. Tu ne repereponds QU'a partir du contexte numéroté fourni ci-dessous. Si le contexte ne
   permet pas de repondre, dis explicitement : "Je ne trouve pas cette information dans ma base."
   N'invente JAMAIS un numéro d'article, un chiffre ou une règle qui n'est pas dans le contexte.
2. Chaque affirmation factuelle doit être rattachee a un numéro d'article present dans le
   contexte (ex: "(article L3141-3)"). N'utilise jamais un numéro d'article absent du contexte.
3. Si la question dépend de la taille de l'entreprise, d'une convention collective, ou de
   circonstances particulières non précisées, donne la règle générale du Code du travail, cite
   l'article, et indique explicitement que la réponse peut varier selon la convention collective
...
4. Si la question demande une APPRECIATION ou une INTERPRETATION au cas par cas (ex: "mon
   ...
   personnel. Rappelle la ou les règles légales générales pertinentes (avec citation), explique
   les criteres que la loi ou la jurisprudence retiennent en général, et indique clairement que
...
def format_context(chunks: list) -> str:
    """Numéroté le contexte pour que le LLM puisse s'y référer sans ambiguite."""
    lines = []
    for i, c in enumerate(chunks, start=1):
        ...
        lines.append(f"[{i}] Article {article_id} - thème: {theme}\n{texte}")
    return "\n\n".join(lines)
...
def generate_answer(question: str, retrieval_result: dict, corpus_date: str = None) -> str:
    """Génère la réponse finale. `retrieval_result` est la sortie de
    ...
    try:
        ...
        raw_answer = (
            "Une erreur technique empêche l'appel au modèle de langage "
...
def assemble_final_answer(answer_text: str, low_confidence: bool, corpus_date: str = None) -> str:
    """Assemble la réponse finale. C'est ICI, en code, que l'avertissement
    ...
    if low_confidence:
        parts.append(
            ...
            "peut-être imparfaitement a votre question. Verifiez sur legifrance.gouv.fr.*"
        )
