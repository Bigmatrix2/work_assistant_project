...
Deux sous-commandes :
    ...
    python -m src.cli chat     -> boucle interactive de questions-réponses
...
REWRITE_SYSTEM_PROMPT = (
    "Tu reformules une question de suivi en une question autonome et complète, "
    "en t'appuyant sur l'historique de conversation fourni. Si la question est "
    "déjà autonome, renvoie-la telle quelle. Reponds uniquement avec la question "
...
def cmd_chat(_args) -> None:
    ...
    print(f"Base indexee le {meta.get('last_index_run')} - {meta.get('total_chunks')} chunks - "
          f"modèle d'embedding : {meta.get('embedding_model')}")
...
def main() -> None:
    ...
    subparsers.add_parser("chat", help="Lance la boucle interactive de questions-réponses.").set_defaults(func=cmd_chat)
