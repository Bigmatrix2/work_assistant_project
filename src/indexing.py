...
Construit :
  ...
  - un fichier index_meta.json qui trace le nom du modèle d'embedding et les
...
Contrainte du sujet : "au redemarrage, le système recharge la base sans
reindexer". C'est pour cela que index_chunks() vérifie d'abord si un chunk
avec le même hash existe déjà avant de l'embedder.
"""
...
def get_embedding_model():
    """Charge le modèle d'embedding (mis en cache pour eviter les rechargements)."""
...
def get_chroma_collection():
    """Récupère (ou crée) la collection Chroma.
    ...
    désaccord de metrique produisait des scores de confiance errones,
    ecrases pres de 0 pour des similarites cosinus reelles moyennes (~0.5) -
    détecte via tests/calibrate_confidence.py (mur de scores a 0.0000 sur
    des questions pourtant dans le corpus).

    ATTENTION : cette metadonnee n'est appliquee qu'A LA CRÉATION de la
    collection. Si une collection existante a ete créée AVANT ce correctif
    (donc avec la metrique L2 par defaut), il faut supprimer le dossier
    data/chroma_db/ et relancer une indexation complète - un simple
    `get_or_create_collection` sur une collection existante ne change pas
    sa metrique retroactivement. La vérification ci-dessous alerte
...
def _existing_hashes(collection) -> dict:
    """Retourne {chunk_id: hash} déjà present dans la collection Chroma."""
    try:
...
def index_chunks(chunks: list, force_reembed: bool = False) -> dict:
    ...
    ou modifiés (hash different), sauf si force_reembed=True.
...
def build_bm25_index(chunks: list) -> None:
    ...
    pas de notion d'upsert incremental fine : on le reconstruit entièrement
    a partir de la liste de chunks (operation rapide, pas d'appel reseau/LLM,
    donc pas de cout significatif même a chaque mise a jour)."""
...
def tokenize(text: str) -> list:
    ...
    et on evite une dependance NLP supplémentaire pour un projet pedagogique."""
    import re
    text = text.lower()
