"""
Jalon 1 - Preparation des données.
...
nettoie, et produit une liste homogene de documents avec :
  - un identifiant stable (numéro d'article)
  - un texte nettoye
  - des metadonnees (thème, titre, source, hash, date de mise a jour)

Le hash sert de base aux mises a jour incrementales (voir update_corpus.py) :
si le hash d'un article n'a pas change depuis la dernière indexation, on
...
def compute_hash(article_id: str, texte: str) -> str:
    """Hash stable utilise pour détecter si un article a change entre deux
...
def build_document(raw_article: dict, source: str) -> dict:
    ...
    - le texte a EMBEDDER contient : numéro d'article + titre + texte de loi,
      pour que la recherche vectorielle beneficie du contexte thematique.
    - le numéro d'article est aussi duplique dans les METADONNEES, qui sont
      la source de vérité utilisée par le prompt de génération pour la
      citation (on ne fait jamais confiance au texte libre pour extraire
      un numéro d'article : on relit toujours les metadonnees du chunk).
    """
...
def quality_check(documents: list, n: int = 10) -> None:
    """Contrôle qualite du sujet (Jalon 1) : affiche n documents au hasard."""
    import random
